...
import { approveVendor, deleteVendor, getVendors } from "../../services/adminManagementService";
...
const VendorsPage = () => {
  ...
  const shown = useMemo(() => { const query = search.trim().toLowerCase(); return paid.filter((item) => (tab === "all" || item.approvalStatus === "pending") && (category === "all" || item.category === category) && (!query || [item.businessName, item.ownerName, item.email, item.location].filter(Boolean).some((value) => value.toLowerCase().includes(query)))); }, [paid, tab, category, search]);
