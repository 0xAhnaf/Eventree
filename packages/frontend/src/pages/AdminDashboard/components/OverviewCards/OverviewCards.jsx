...
import { Wallet, CalendarCheck, Building2, Clock3 } from "lucide-react";
...
const AnimatedNumber = ({ value, currency = false }) => {
  ...
  return (
    <>{currency ? formatCurrency(count) : count.toLocaleString("en-BD")}</>
  );
};
      ...
      {cards.map((card, index) => (
        <div className="overview-card" key={index}>
          ...
          <div className="overview-content">
            ...
            <h2>
              <AnimatedNumber value={card.value} currency={card.currency} />
            </h2>
